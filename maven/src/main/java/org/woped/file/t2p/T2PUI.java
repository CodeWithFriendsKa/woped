...
public class T2PUI extends JDialog {
	...
	private void httpBackgroundWorker(final String text) {
		if (bgTask != null && !bgTask.isDone()) return;


		final String url = "http://" + ConfigurationManager.getConfiguration().getText2ProcessServerHost() + ":" + ConfigurationManager.getConfiguration().getText2ProcessServerPort() + ConfigurationManager.getConfiguration().getText2ProcessServerURI() + "/generate";
